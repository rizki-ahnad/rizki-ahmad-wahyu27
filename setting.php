      ...
      <div class="bg-secondary mb-4" style="width:1263px; height:8px;"></div>
        ...
        <div class="row ms-5">
            <div class="col-lg-6 ms-5">
                <h2 class="page-header">Setting Form</h2>
                    <div class="panel panel-default mt-5 mb-5">
                        <div class="panel-heading mb-2 ms-5">
                            Setting Aktif dan Nonaktif Form Pendaftaran & Seleksi
                        </div>
                        <!-- /.panel-heading -->
                        <div class="panel-body">
                          <div class="d-flex">
                            ...
                            <form action="" method="POST">
                              <?php
                              $id_form = 1;
                              $sql="SELECT * FROM formcontrol WHERE id='$id_form'";
                              $perintah=mysqli_query($con,$sql);
                              $row=mysqli_fetch_assoc($perintah);
                              if(mysqli_num_rows($perintah)===1){
                                $formAktif = $row['formaktif'];
                              }                              
                              ?>
                              <?php
                              $id_form1 = 2;
                              $sql1="SELECT * FROM formcontrol WHERE id='$id_form1'";
                              $perintah1=mysqli_query($con,$sql1);
                              $row1=mysqli_fetch_assoc($perintah1);
                              if(mysqli_num_rows($perintah1)===1){
                                $formAktif1 = $row1['formaktif'];
                              }                              
                              ?>
                              <div class="form-group mb-3">
                                    <label>Pengaturan Aktif/Tidak Form Pendaftaran</label>
                                    <select class="form-control" name="status_form">
                                            <option value="<?= $formAktif;?>"><?= $formAktif;?></option>
                                            <option value="yes">yes</option>
                                            <option value="no">no</option>
                                            </select>
                                    </div>
                                    <div class="form-group mb-3">
                                  <label>Pengaturan Aktif/Tidak Form Seleksi</label>
                                  <select class="form-control" name="status">
                                          <option value="<?= $formAktif1;?>"><?= $formAktif1;?></option>
                                          <option value="yes">yes</option>
                                          <option value="no">no</option>
                                          </select>
                                  </div>
                                ...
                                if(isset($_POST['update'])){
                                    $status_form1 = $_POST['status_form'];
                                    $status_form2 = $_POST['status'];
                                    $id1 = 1;
                                    $id2 = 2;
                                    $sql_form = mysqli_query($con,"UPDATE formcontrol SET formaktif='$status_form1' WHERE id='$id1'");
                                    $sql_form .= mysqli_query($con,"UPDATE formcontrol SET formaktif='$status_form2' WHERE id='$id2'");
                                    // $query_form = mysqli_query($con, $sql_form);
                                    if($sql_form){
                                        echo "<script>alert('Setting Form Berhasil');window.location='';</script>";
                                    }
                                    else{
                                        echo "<script>alert('gagal setting form');</script>";
                                    }
                                }
                                ?>
                            </form>
