...
<main>
  ...
  <div class="d-block">
    ...
    <div class="mt-3" style="margin-left:3px;">
      ...
      <div class="ms-5">
        <center>
          <h2>Form Kalender Amik Purnama Niaga Indramayu</h2>
        </center>
        <a href="" class="btn btn-primary ms-5 mb-2" type="submit" onclick="toggleElement()">Add</a>
      </div>   
      <table id="myScrollTable" border="0" cellpadding="5" cellspacing="4" align="center">
        <tbody class="rounded">
              <tr bgcolor="#2a7ed2" style="color:#ffffff;">
                <th style="width:60px; text-align:center;">No</th>
                <th style="width:250px;">Nama Jadwal</th>
                <th style="width:110px;">Dari Hari</th>
                <th style="width:160px; text-align:center;">Ke Hari</th>
                <th style="width:200px; text-align:center;">Dari Tanggal</th>
                <th style="width:260px; text-align:center;">Ke tanggal</th>
                <th style="width:60px;">Aksi</th>
              </tr>
            <?php
                include "koneksi.php";
                error_reporting(0);
                $data_user= "SELECT * FROM jadwal";
                ...
                <tr bgcolor="#7fb5ec">
                  ...
                  <td><?php echo $data['hari_awal']; ?></td>
                  <td style="text-align:center;"><?php echo $data['hari_akhir']; ?></td>
                  <td style="text-align:center;"><?php echo $data['tgl_mulai']; ?></td>
                  <td style="text-align:center;"><?php echo $data['tgl_selesai']; ?></td>
                  <td>
                    <a href="edit_jadwal.php?id=<?= $data['id_jadwal'];?>" class="btn btn-primary" type="submit"><i class="bi bi-pencil-square"></i></a>
                  </td>
                </tr>
                <?php
                }
          ...
          <div id="elemen" style="position: absolute; display:block; width:700px; height:500px; margin-top:-470px; margin-left:260px;">
          <div id="elemen" class="alert alert-info alert-dismissible fade show" role="alert">
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            <form action="" method="POST">           
                <div class="mb-3">
                    <label for="formGroupExampleInput2" class="form-label">Nama Jadwal</label>
                    <input type="text" class="form-control" id="formGroupExampleInput2" required name="nama_jadwal" placeholder="isi nama mahasiswa">
                </div>
                <div class="mb-3">
                    <label for="formGroupExampleInput2" class="form-label">Dari Hari</label>
                    <input type="text" class="form-control" id="formGroupExampleInput2" required name="hari_awal" placeholder="isi email mahasiswa" >
                </div>
                <div class="mb-3">
                    <label for="formGroupExampleInput2" class="form-label">Ke Hari</label>
                    <input type="text" class="form-control" id="formGroupExampleInput2" required name="hari_akhir" placeholder="isi hasil seleksi calon mahasiswa">
                </div>
                <div class="mb-3">
                    <label for="formGroupExampleInput2" class="form-label">Tanggal Mulai</label>
                    <input type="date" class="form-control" id="formGroupExampleInput2" required name="tgl_mulai">
                </div>
                <div class="mb-3">
                    <label for="formGroupExampleInput2" class="form-label">Tanggal Selesai</label>
                    <input type="date" class="form-control" id="formGroupExampleInput2" required name="tgl_selesai">
                </div>
                <div class="d-flex">
                  <button class="btn btn-outline-warning me-1" type="submit" name="tambah">Add</button>
                </div>
                <?php
                if(isset($_POST['tambah'])){
                  $nama_jdwl1 = $_POST['nama_jadwal'];
                  $h_a1 = $_POST['hari_awal'];
                  $h_s1 = $_POST['hari_akhir'];
                  $tgl_a1 = $_POST['tgl_mulai'];
                  $tgl_s1 = $_POST['tgl_selesai'];
                  $dataSeleksi = mysqli_query($con,"SELECT * FROM jadwal WHERE nama_jadwal='$nama_jdwl1'");
                  // menghitung jumlah data yang ditemukan
                  $cekSeleksi = mysqli_num_rows($dataSeleksi);
                  
                  if($cekSeleksi > 0){
                      echo "<script>alert('tidak boleh mengirim 2x!');</script>";
                  }else{
                  $a1 = mysqli_query($con,"INSERT INTO jadwal (nama_jadwal, hari_awal, hari_akhir, tgl_mulai, tgl_selesai)VALUES('$nama_jdwl1', '$h_a1' , '$h_s1', '$tgl_a1', '$tgl_s1')");
                  
                  if($a1){
                      echo "<script>alert('Berhasil mengirim!');window.location='buat_jadwal.php';</script>";
                  }else{
                      echo "<script>alert('silahkan coba lagi!');</script>";
                  }
                  // var_dump($nama_jdwl, $h_a, $h_s, $tgl_a, $tgl_s);
                  }                      
                  }
                ?>
            </form>
