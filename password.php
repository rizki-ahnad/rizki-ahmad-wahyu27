...
<main>
  ...
  <div class="d-block">
    ...
    <div class="mt-1" style="margin-left:3px;">
      ...
      <div class="ms-5">
        <a href="password.php" onclick="toggleElement()" class="btn btn-outline-dark ms-5 mb-1">Add User</a>
      </div>
      <div class="mt-1 rounded">
      <table id="myScrollTable" border="0" cellpadding="5" cellspacing="4" align="center">
        <tbody class="rounded">
        <tr bgcolor="#009966" style="color:#fff">
        ...
        <?php
                ...
                if(isset($_POST['cari'])){
                    $cari = $_POST['cari'];
                    $data_user= "SELECT * FROM user WHERE nama like '%".$cari."%' OR email like '%".$cari."%' OR level like '%".$cari."%'";
                    $hasil = mysqli_query($con, $data_user);
                  }else{
                    $data_user= "SELECT * FROM user";
                    $hasil = mysqli_query($con, $data_user);
                ...
                <tr>
                  ...
                  <td  style="text-align:center;">
                      <a href="setting_password.php?id=<?= $data['id_user']?>" target="_balnk" class="btn btn-info"><i class="bi bi-key" style="width:35px; height:35px;"></i></a>
                    </td>
                </tr>
                <?php
                }
    ...
    <div id="elemen" style="position: absolute; width:600px; height:300px; margin-left:330px; margin-top:-380px;">
            ...
            <form action="" method="POST"> 
                <div class="d-flex">
                  <div class="mb-3 me-2">
                      ...
                      <input type="text" class="form-control" id="formGroupExampleInput2" name="nama" required>
                  </div> 
                  <div class="mb-3 me-2">
                      <label for="formGroupExampleInput2" class="form-label">Email</label>
                      <input type="email" class="form-control" id="formGroupExampleInput2" required name="email">
                  </div>
                  <div class="mb-3 me-2">
                    <label for="formGroupExampleInput2" class="form-label">No Hp</label>
                    <input type="number" class="form-control" id="formGroupExampleInput2" required name="nohp">
                  </div>
                </div>
                <div class="d-flex">
                  <div class="mb-3 me-2">
                    <label for="formGroupExampleInput2" class="form-label">Password</label>
                    <input type="password" class="form-control" id="formGroupExampleInput2" required name="password">
                  </div>   
                  <div class="mb-3">
                    <label for="formGroupExampleInput2" class="form-label">Level</label>
                    <select class="form-select" aria-label="Default select example" required name="level" required>
                ...
                <button class="btn btn-outline-dark" type="submit" name="add">Add</button>
                <?php
                  include "koneksi.php";
                  if(isset($_POST['add'])){
                    $nama1 = $_POST['nama'];
                    ...
                    $level1 = $_POST['level'];
                    // var_dump($password);
                // var_dump($us, $email, $nohp, $password, $level);
                    $dataUser = mysqli_query($con,"SELECT * FROM user WHERE email='$email1' ");

                    // menghitung jumlah dataUser yang ditemukan
                    $cekUser = mysqli_num_rows($dataUser);
                    
                    if($cekUser > 0){
                        echo "<script>alert('gagal brow!');</script>";
                    }else{
                        $sqlUser = mysqli_query($con,"INSERT INTO user (nama, email, nohp, password, level)
                        VALUES('$nama1', '$email1', '$nohp1', '$password1', '$level1')");

                        if($sqlUser){
                            echo "<script>alert('Add Succes!');window.location.href='password.php'</script>";
                        }else{
