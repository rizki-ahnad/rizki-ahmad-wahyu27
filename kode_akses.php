...
<?php
 ...
 $query = mysqli_query($koneksi, "SELECT max(code_akses) as kodeTerbesar FROM kode_akses");
 $data = mysqli_fetch_array($query);
 $kodeAkses = $data['kodeTerbesar'];
 
 // mengambil angka dari kode barang terbesar, menggunakan fungsi substr
 // dan diubah ke integer dengan (int)
 $urutan = (int) substr($kodeAkses, 3, 3);
...
<main>
  ...
  <div class="d-block">
    ...
    <div class="mt-3" style="margin-left:3px;">
      ...
      <center>
      <div class="mt-5 mb-5">
        <h2>Form Pembuatan Kode Akses</h2>
      </div>
      </center>
      <table id="myScrollTable" border="0" cellpadding="5" cellspacing="4" align="center">
        <tbody>
              <tr bgcolor="#f08741">
                ...
                <th style="width:260px;">Foto Bukti Trnasfer</th>
                <th style="width:260px; text-align:center;">Status</th>
                <th style="width:200px; text-align:center;">Aksi</th>
              </tr>
            <?php
                include "koneksi.php";
                $data_seleksi    =mysqli_query($con, "SELECT * FROM bukti_bayar ORDER BY tanggal DESC");
                $jumlah_data =mysqli_num_rows($data_seleksi);
                $no = 1;
                while($data = mysqli_fetch_assoc($data_seleksi)){
            ?>
                <tr bgcolor="#f2b185">
                  ...
                  <td style=""><a href="img/<?= $data['foto_bukti']  ?>" target="blank" style="text-decoration:none" class="link-primary"><?= $data['foto_bukti']  ?></a></td>
                  <td style="text-align:center;"><?php
                      $id_p = $data['id_pengguna'];
                      $nama_bukti = $data['nama'];
                      $queryData= "SELECT * FROM kode_akses WHERE id_pengguna='$id_p'";
                      ...
                      while ($fill1 = mysqli_fetch_assoc($hasil_data)){
                        ...
                        if($nama_bukti != $nama1){
                          $sts = "kode belum di buat";
                          echo $sts;
                        }else{
                          $sts = "kode sudah di buat";
                          echo $sts;
                      }
                  ?></td>
                  <td style="text-align:center;">
                      <a href="buat_kode.php?id=<?= $data['id_bukti']?>" target="_balnk" class="btn btn-outline-light"><img src="gambar/tambah.png" alt="tambah"> | Add Code</a>
                    </td>
                </tr>
                <?php
                }
