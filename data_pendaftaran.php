  ...
  <div class="d-block">
    ...
    <div class="mt-3" style="margin-left:3px;">
      ...
      <div class="alert alert-dark mt-2 d-flex p-3" role="alert">
        <form action="data_pendaftaran.php" method="get">
          ...
          <div class="d-flex ms-5">
                  <form action="" method="POST">
                        <div class="input-group mb-3 ms-5 me-5">
                            ...
                            <a href="lap_pendaftaran.php?cari=<?php if(isset($_POST['cari'])){echo $_POST['cari'];}?>" target="_blank" class="btn btn-success ms-3"><i class="bi bi-printer"></i></a>
                        </div>
                  </form>
        ...
        <tbody class="rounded">
              <tr bgcolor="#fff" style="color:#00000;">
                <th style="width:60px; text-align:center;">No</th>
                <th style="width:160px;">No Daftar</th>
                <th style="width:210px;">Nama</th>
                <th style="width:160px;">Dokumen STTB</th>
                <th style="width:160px;">Dokumen DANUN</th>
                <th style="width:160px;">Dokumen KTP</th>
                <th style="width:160px;">Dokumen Akta</th>
                <th style="width:160px;">Dokumen Foto</th>
              </tr>
            <?php
                include "koneksi.php";
                error_reporting(0);
                $data_user= "SELECT * FROM pendaftaran";
                $hasil = mysqli_query($con, $data_user);
                $col = mysqli_num_rows($hasil);
                $no = 1;
                if(isset($_POST['cari'])){
                    $cari = $_POST['cari'];
                    $data_user= "SELECT * FROM pendaftaran WHERE nama like '%".$cari."%' OR nomor_daftar like '%".$cari."%'";
                    $hasil = mysqli_query($con, $data_user);
                  }elseif(isset($_GET['dari']) && isset($_GET['ke'])){
                        $data = "SELECT * FROM pendaftaran WHERE tanggal BETWEEN '".$_GET['dari']."' and '".$_GET['ke']."'";
                        $hasil = mysqli_query($con, $data);
                  }elseif(isset($_POST['refresh'])){
                      $data_user= "SELECT * FROM pendaftaran";
                      $hasil = mysqli_query($con, $data_user);
                      echo "<script>window.location='data_pendaftaran.php';</script>";
                  }else{
                    $data = "SELECT * FROM pendaftaran";
                    $hasil = mysqli_query($con, $data);		
                  }
                ...
                <tr bgcolor="#fff">
                  ...
                  <td style="width:60px;"><?php echo $data['nomor_daftar']; ?></td>
                  <td><?php echo $data['nama'];?></td>
                  <td><a href="file/<?=$data['sttb']?>" target="blank" style="text-decoration:none" class="link-primary"><?=$data['sttb']?></a></td>
                  <td><a href="file/<?=$data['danun']?>" target="blank" style="text-decoration:none" class="link-primary"><?=$data['danun']?></a></td>
                  <td><a href="file/<?=$data['ktp']?>" target="blank" style="text-decoration:none" class="link-primary"><?=$data['ktp']?></a></td>
                  <td><a href="file/<?=$data['akte']?>" target="blank" style="text-decoration:none" class="link-primary"><?=$data['akte']?></a></td>
                  <td><a href="file/<?=$data['poto_2']?>" target="blank" style="text-decoration:none" class="link-primary"><?=$data['poto_2']?></a></td>
                </tr>
                <?php
                }
                ?>
