...
<?php
 ...
 $query = mysqli_query($con,"SELECT max(kode_jabatan) as nimTerbesar FROM pegawai");
 ...
 // bilangan yang diambil ini ditambah 1 untuk menentukan nomor urut berikutnya
  $urutan++;
  $huruf = "PGW-";
  $kode_jabatan = $huruf. sprintf("%04s", $urutan);
...
<main>
  ...
  <div class="d-block">
    ...
    <div class="mt-3" style="margin-left:3px;">
      ...
      <div class="mb-5">
          <center><h2>Form Tambah User & Data Pegawai</h2></center>
      </div>
      <div class="container">
        <form action="" method="POST">           
            <div class="d-flex">
                <div class="ms-4" style="width:500px; height:300px;">
                    <div class="mb-3">
                        <label for="formGroupExampleInput2" class="form-label">Nama</label>
                        <input type="text" class="form-control" id="formGroupExampleInput2" required name="nama" placeholder="isi nama lengkap">
                    </div>
                    <div class="mb-3">
                        <label for="formGroupExampleInput2" class="form-label">Email</label>
                        <input type="email" class="form-control" id="formGroupExampleInput2" required name="email" placeholder="isi email aktif" >
                    </div>
                    <div class="mb-3">
                        <label for="formGroupExampleInput2" class="form-label">No Hp</label>
                        <input type="number" class="form-control" id="formGroupExampleInput2" required name="nohp" placeholder="isi no hp atau wa aktif">
                    </div>
                </div>
                <div class="ms-5" style="width:500px; height:300px;">
                    <div class="mb-3">
                        ...
                        <select class="form-select" aria-label="Default select example" name="jabatan" required>
                            ...
                            <option value="Wakil Direktur">Wakil Dirut</option>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="hidden" class="form-control" id="formGroupExampleInput2" name="kode_jabatan" value="<?= $kode_jabatan ?>">
                        <input type="text" class="form-control" id="formGroupExampleInput2" value="<?= $kode_jabatan ?>" disabled>
                    </div>
                    <div>
                    <button class="btn btn-outline-primary me-1" type="submit" name="tambah">Add</button>
                    </div>
                </div>
            </div>    
                <?php
                if(isset($_POST['tambah'])){
                    $nama_pgw = $_POST['nama'];
                    $email_pgw = $_POST['email'];
                    $nohp_pgw = $_POST['nohp'];
                    $jabatan2 = $_POST['jabatan'];
                    $jabatan3 = $_POST['kode_jabatan'];
                    $tanggal = date('Y-m-d');
                    // var_dump($password);
                // var_dump($nama_pgw, $email_pgw, $nohp_pgw, $password, $level, $jabatan2, $jabatan3, $tanggal);

                    $data_pgw = mysqli_query($con,"SELECT * FROM pegawai WHERE email='$email_pgw' ");

                    // menghitung jumlah data_pgw yang ditemukan
                    $cek_pgw = mysqli_num_rows($data_pgw);
                    
                    if($cek_pgw > 0){
                        echo "<script>alert('gagal brow!');</script>";
                    }else{
                        // $q = mysqli_query($con,"INSERT INTO user (nama, email, nohp, password, level)VALUES('$nama_pgw', '$email_pgw', '$nohp_pgw', '$password', '$level')");

                        $q = mysqli_query($con,"INSERT INTO pegawai (nama, email, nohp, jabatan, kode_jabatan, tanggal) VALUES('$nama_pgw', '$email_pgw', '$nohp_pgw', '$jabatan2', '$jabatan3', '$tanggal')");

                        // $q = mysqli_multi_query($con, $sql_pgw);

                        if($q){
                            echo "<script>alert('Tambah Data Succes!');window.location.href='pegawai.php'</script>";
                        }else{
                            echo "<script>alert('maaf anda gagal tambah data, silahkan coba lagi!');</script>";
                        }
                    }                     
                  }
                ?>
