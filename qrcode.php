  ...
  <div class="d-block">
    ...
    <div class="mt-3" style="margin-left:3px;">
      ...
      <div class="container align-items-center p-2" style="width: 350px;">
        <div class="card">
            <div class="card-body">
              <p class="card-title text-center"><b>Membuat QR Code Maps</b></p>
              <fieldset>
                <form method="post" action="" class="text-center">
                <input type="text" class="form-control mb-2"  required placeholder="masukan judul kode" name="judul">
                    ...
                    if (isset($_POST['generate'])){
                        ...
                        mkdir($tempdir, 0755);
                        $file_name= $judul.date("Ymd").".png";	
                        $file_path = $tempdir.$file_name;

                        $qrcode = mysqli_query($con,"SELECT * FROM tbl_qrcode WHERE judul='$judul'");
                        $query_code = mysqli_fetch_assoc($qrcode);
                        $cek = mysqli_num_rows($qrcode);
                        if($cek > 0){
                            echo "<script>alert('nama judul sudah ada!');</script>";
                        }else{
                            $sql = mysqli_query($con,"INSERT INTO tbl_qrcode (judul, tanggal, qrcode)
                            VALUES('$judul', '$tanggal' , '$file_name')");
            
                            if($sql){
                                echo "<script>alert('Berhasil Buat QR Code!');window.location.href='halaman_admin.php'</script>";
                            }else{
                        ...
                        echo "<p><img src='".$file_path."' style='width:150px; height:150px;'/></p>";
                        echo "<p>Copyright © </p>" .date('Y');
                    }
                    ?>
                </form>
               </fieldset>
