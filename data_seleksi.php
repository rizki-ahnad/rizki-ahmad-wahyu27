  ...
  <div class="d-block">
    ...
    <div class="mt-3" style="margin-left:3px;">
      ...
      <div class="alert alert-dark mt-2 d-flex p-3" role="alert">
        <form action="data_seleksi.php" method="get">
          ...
          <div class="d-flex ms-5">
                  <form action="" method="POST">
                        <div class="input-group mb-3 ms-5 me-5">
                            ...
                            <a href="lap_seleksi.php?ke=<?= $_GET['ke'];?>" target="_blank" class="btn btn-success ms-3"><i class="bi bi-printer"></i></a>
                        </div>
                  </form>
                  <div class="ms-5 d-flex">
                  <div class="ms-2">
                  <form action="" method="POST">
                    <button class="btn btn-outline-info" type="submit" name="refresh"><i class="bi bi-arrow-repeat"></i></i></button>
                    <a href="rangking.php" target="_blank" class="btn btn-warning ms-1"><i class="bi bi-printer"></i></a>
                  </form>
                  </div>
                  </div>
              </div>
      ...
      <div class="alert alert-info mt-1" role="alert">
        ...
        <tbody class="rounded">
              <tr bgcolor="#8566a3" style="color:#ffffff;">
                <th style="width:60px; text-align:center;">No</th>
                <th style="width:250px;">Nama Mahasiswa</th>
                <th style="width:310px;">Email</th>
                <th style="width:160px; text-align:center;">Hasil Seleksi</th>
                <th style="width:100px;">Status</th>
                <th style="width:160px; text-align:center;">Tanggal Seleksi</th>
              </tr>
            <?php
                include "koneksi.php";
                error_reporting(0);
                $data_user= "SELECT * FROM hasil_seleksi";
                $hasil = mysqli_query($con, $data_user);
                $col = mysqli_num_rows($hasil);
                $no = 1;
                if(isset($_POST['cari'])){
                    $cari = $_POST['cari'];
                    $data_user= "SELECT * FROM hasil_seleksi WHERE nama like '%".$cari."%' OR email like '%".$cari."%' OR hasil like '%".$cari."%' OR tanggal like '%".$cari."%' OR status like '%".$cari."%'";
                    $hasil = mysqli_query($con, $data_user);
                  }elseif(isset($_GET['dari']) && isset($_GET['ke'])){
                        $data = "SELECT * FROM hasil_seleksi WHERE tanggal BETWEEN '".$_GET['dari']."' and '".$_GET['ke']."'";
                        $hasil = mysqli_query($con, $data);
                  }elseif(isset($_POST['refresh'])){
                      $data_user= "SELECT * FROM hasil_seleksi";
                      $hasil = mysqli_query($con, $data_user);
                      echo "<script>window.location='data_seleksi.php';</script>";
                  }else{
                    $data = "SELECT * FROM hasil_seleksi";
                    $hasil = mysqli_query($con, $data);		
                  }
                ...
                <tr bgcolor="#d2b7eb">
                  ...
                  <!-- <td>
                    <a href="data_seleksi.php?id=<?= $data['id_mhs'];?>" class="btn btn-primary" type="submit" onclick="toggleElement()"><i class="bi bi-pencil-square"></i></a>
                  </td> -->
                  <?php
              include("koneksi.php");
              ...
              $sql_seleksi = "SELECT * FROM hasil_seleksi WHERE id_mhs='$id_seleksi'";
              $query_seleksi = mysqli_query($con, $sql_seleksi);
              $calon_mhs = mysqli_fetch_assoc($query_seleksi);
    ...
    <!-- <div id="elemen" style="position: absolute; display:block; width:700px; height:500px; margin-left:260px; margin-top:-410px;">
          <div id="elemen" class="alert alert-warning alert-dismissible fade show" role="alert">
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            <form action="" method="POST">           
                <div class="mb-3">
                    <label for="formGroupExampleInput2" class="form-label">Nama</label>
                    <input type="text" class="form-control" id="formGroupExampleInput2" required name="nama" placeholder="isi nama mahasiswa" value="<?= $calon_mhs['nama']; ?>">
                    <input type="hidden" class="form-control" id="formGroupExampleInput2" required name="id_mahasiswa" placeholder="isi nama mahasiswa" value="<?= $id_seleksi ?>">
                </div>
                <div class="mb-3">
                    <label for="formGroupExampleInput2" class="form-label">Email</label>
                    <input type="text" class="form-control" id="formGroupExampleInput2" required name="email" placeholder="isi email mahasiswa" value="<?= $calon_mhs['email']; ?>">
                </div>
                <div class="mb-3">
                    <label for="formGroupExampleInput2" class="form-label">Hasil Seleksi</label>
                    <input type="text" class="form-control" id="formGroupExampleInput2" required name="hasil" placeholder="isi hasil seleksi calon mahasiswa" value="<?= $calon_mhs['hasil']; ?>">
                </div>
                <div class="mb-3">
                    <label for="formGroupExampleInput2" class="form-label">Status</label>
                    <input type="text" class="form-control" id="formGroupExampleInput2" required name="status" placeholder="isi status seleksi" value="<?= $calon_mhs['status']; ?>">
                </div>
                <button class="btn btn-outline-warning me-5" type="submit" name="update">Update</button>
                <?php
                  include "koneksi.php";
                  if(isset($_POST['update'])){
                    ...
                    $status1 = $_POST['status'];
                      if(empty($id)){
                        ...
                        $sql = "UPDATE hasil_seleksi SET nama='$nama1', email='$email1', hasil='$hasil1', status='$status1' WHERE id_mhs='$id'";
                        $query = mysqli_query($con, $sql);
                        if($query){
                          echo "<script>alert('Berhasil di ubah');window.location='data_seleksi.php';</script>";
                        }else{
                          echo "<script>alert('Gagal di di ubah');</script>";
                        }
                      }
                  }
                ?>
