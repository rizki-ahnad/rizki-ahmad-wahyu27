          ...
          <div class="d-flex ms-5">
                  <form action="" method="POST">
                        <div class="input-group mb-3 ms-5 me-5">
                            ...
                            <a href="lap_pegawai.php?ke=<?= $_GET['ke'];?>" target="_blank" class="btn btn-success rounded ms-5"><i class="bi bi-printer"></i></a>
                            <a href="tambah_pegawai.php" class="btn btn-outline-danger rounded ms-2"><i class="bi bi-person-plus"></i></i></a>
                        </div>
                  </form>
      ...
      <div class="alert alert-info mt-1" role="alert">
        ...
        <tbody class="rounded">
              <tr bgcolor="#ffffff" style="color:#000;">
                ...
                <th style="width:350px;">Nama Pegawai</th>
                <th style="width:110px;">Jabatan</th>
                <th style="width:160px; text-align:center;">Kode Jabatan</th>
                <th style="width:260px; text-align:center;">Tanggal Masuk</th>
                <th style="width:160px; text-align:center;">Aksi</th>
              </tr>
            <?php
                include "koneksi.php";
                error_reporting(0);
                $data_user= "SELECT * FROM pegawai";
                $hasil = mysqli_query($con, $data_user);
                $col = mysqli_num_rows($hasil);
                $no = 1;
                if(isset($_POST['cari'])){
                    $cari = $_POST['cari'];
                    $data_user= "SELECT * FROM pegawai WHERE nama like '%".$cari."%' OR jabatan like '%".$cari."%' OR kode_jabatan like '%".$cari."%' OR tanggal like '%".$cari."%'";
                    $hasil = mysqli_query($con, $data_user);
                  }elseif(isset($_GET['dari']) && isset($_GET['ke'])){
                        $data = "SELECT * FROM pegawai WHERE tanggal BETWEEN '".$_GET['dari']."' and '".$_GET['ke']."'";
                ...
                <tr bgcolor="#ffffff" style="color:#000">
                  ...
                  <td style="text-align:center;"><?php echo $data['kode_jabatan']; ?></td>
                  <td style="text-align:center;"><?php echo $data['tanggal']; ?></td>
                  <td style="text-align:center;">
                    <a href="pegawai.php?id=<?= $data['id_pegawai'];?>" class="btn btn-secondary" type="submit" onclick="toggleElement()"><i class="bi bi-pencil-square"></i></a>
                  </td>
                  <?php
              include("koneksi.php");
              ...
              $sql_seleksi = "SELECT * FROM pegawai WHERE id_pegawai='$id_pegawai'";
              $query_seleksi = mysqli_query($con, $sql_seleksi);
              $pegawai = mysqli_fetch_assoc($query_seleksi);
          ...
          <div id="elemen" class="alert alert-warning alert-dismissible fade show" role="alert">
            ...
            <form action="" method="POST">           
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="formGroupExampleInput2" required name="nama" value="<?= $pegawai['nama']; ?>">
                    <input type="hidden" class="form-control" id="formGroupExampleInput2" required name="id_pegawai" value="<?= $id_pegawai ?>">
                </div>
                <div class="mb-3">
                ...
                <div class="mb-3">
                    <label for="formGroupExampleInput2" class="form-label">Kode Jabatan</label>
                    <input type="hidden" class="form-control" id="formGroupExampleInput2" name="kode_jabatan" value="<?= $pegawai['kode_jabatan']; ?>">
                    <input type="text" class="form-control" id="formGroupExampleInput2" value="<?= $pegawai['kode_jabatan']; ?>" disabled>
                </div>
                <div class="mb-3">
                    <label for="formGroupExampleInput2" class="form-label">Status</label>
                    <input type="date" class="form-control" id="formGroupExampleInput2" name="tanggal" value="<?= $pegawai['tanggal']; ?>">
                </div>
                  ...
                  if(isset($_POST['update'])){
                    ...
                    $tanggal = $_POST['tanggal'];
                      if(empty($id)){
                        ...
                        $sql = "UPDATE pegawai SET nama='$nama1', jabatan='$jabatan', kode_jabatan='$kode_jabatan', tanggal='$tanggal' WHERE id_pegawai='$id'";
                        $query = mysqli_query($con, $sql);
                        if($query){
                          echo "<script>alert('Berhasil di ubah');window.location='pegawai.php';</script>";
                        }else{
