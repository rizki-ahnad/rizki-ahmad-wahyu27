  ...
  <div class="d-block">
    ...
    <div class="mt-3" style="margin-left:3px;">
      ...
      <div class="alert alert-dark mt-2 d-flex p-3" role="alert">
        <form action="data_pembayaran.php" method="get">
          ...
          <div class="d-flex ms-5">
                  <form action="" method="POST">
                        <div class="input-group mb-3 ms-5 me-5">
                            ...
                            <a href="lap_pembayaran.php?ke=<?= $_GET['ke'];?>" target="_blank" class="btn btn-success ms-3"><i class="bi bi-printer"></i></a>
                        </div>
                  </form>
                  <div class="ms-5 d-flex">
                  <div class="ms-2">
                  <form action="" method="POST">
                    <button class="btn btn-outline-info me-5" type="submit" name="refresh"><i class="bi bi-arrow-repeat"></i></i></button>
                  </form>
                  </div>
                  </div>
              </div>
      ...
      <div class="alert alert-info mt-1" role="alert">
        ...
        <tbody class="rounded">
              <tr bgcolor="#fff" style="color:#000000;">
                <th style="width:60px; text-align:center;">No</th>
                <th style="width:210px;">Nama</th>
                <th style="width:210px;">Email</th>
                <th style="width:160px; text-align:center;">No Hp</th>
                <th style="width:100px;">Bayar Pendaftaran</th>
                <th style="width:160px; text-align:center;">Status Pembayaran</th>
                <th style="width:160px; text-align:center;">Tanggal Bayar</th>
              </tr>
            <?php
                include "koneksi.php";
                error_reporting(0);
                $data_user= "SELECT * FROM kode_akses";
                $hasil = mysqli_query($con, $data_user);
                $col = mysqli_num_rows($hasil);
                $no = 1;
                if(isset($_POST['cari'])){
                    $cari = $_POST['cari'];
                    $data_user= "SELECT * FROM kode_akses WHERE nama like '%".$cari."%' OR email like '%".$cari."%' OR bayar_daftar like '%".$cari."%' OR status like '%".$cari."%' OR tanggal like '%".$cari."%'";
                    $hasil = mysqli_query($con, $data_user);
                  }elseif(isset($_GET['dari']) && isset($_GET['ke'])){
                        $data = "SELECT * FROM kode_akses WHERE tanggal BETWEEN '".$_GET['dari']."' and '".$_GET['ke']."'";
                ...
                <tr bgcolor="#fff" style="color:#000000;">
                  ...
                  <td><?php echo $data['bayar_daftar']; ?></td>
                  <td style="text-align:center;"><?php echo $data['status']; ?></td>
                  <td style="text-align:center;"><?php echo $data['tanggal']; ?></td>
                  <!-- <td>
                    <a href="data_pembayaran.php?id=<?= $data['id_kode'];?>" class="btn btn-light" type="submit" onclick="toggleElement()"><i class="bi bi-pencil-square"></i></a>
                  </td> -->
                  <?php
              include("koneksi.php");
              ...
              $sql_seleksi = "SELECT * FROM kode_akses WHERE id_kode='$id_seleksi'";
    ...
    <!-- <div id="elemen" style="position: absolute; display:block; width:700px; height:500px; margin-left:260px; margin-top:-410px;">
          <div id="elemen" class="alert alert-light alert-dismissible fade show" role="alert">
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            <form action="" method="POST">           
                <div class="mb-3">
                    <label for="formGroupExampleInput2" class="form-label">Nama</label>
                    <input type="text" class="form-control" id="formGroupExampleInput2" required name="nama" value="<?= $calon_mhs['nama']; ?>">
                    <input type="hidden" class="form-control" id="formGroupExampleInput2" required name="id_kode" value="<?= $id_seleksi ?>">
                </div>
                <div class="mb-3">
                    <label for="formGroupExampleInput2" class="form-label">Bayar Pendaftaran Online</label>
                    <input type="text" class="form-control" id="formGroupExampleInput2" required name="bayar_daftar" value="<?= $calon_mhs['bayar_daftar']; ?>">
                </div>
                <div class="mb-3">
                    <label for="formGroupExampleInput2" class="form-label">Status Pembayaran</label>
                    <input type="text" class="form-control" id="formGroupExampleInput2" required name="status"value="<?= $calon_mhs['status']; ?>">
                </div>
                <div class="mb-3">
                    <label for="formGroupExampleInput2" class="form-label">Tanggal Bayar</label>
                    <input type="date" class="form-control" id="formGroupExampleInput2" required name="tanggal" value="<?= $calon_mhs['tanggal']; ?>">
                </div>
                ...
                <?php
                  ...
                  if(isset($_POST['update'])){
                    $id = $_POST['id_kode'];
                    $nama1 = $_POST['nama'];
                    $bayar1 = $_POST['bayar_daftar'];
                    $status1 = $_POST['status'];
                    $tanggal1 = $_POST['tanggal'];
                      if(empty($id)){
                        echo "<script>alert('data tidak ditemukan');</script>";
                      }else{
                        $sql1 = "UPDATE kode_akses SET nama='$nama1', bayar_daftar='$bayar1', status='$status1', tanggal='$tanggal1' WHERE id_kode='$id'";
                        $query1 = mysqli_query($con, $sql1);
                        if($query1){
                          echo "<script>alert('Berhasil di ubah');window.location='data_pembayaran.php';</script>";
                        }else{
