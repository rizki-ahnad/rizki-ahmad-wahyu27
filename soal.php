...
<main>
  ...
  <div class="d-block">
    ...
    <div class="mt-3" style="margin-left:3px;">
      <div class="bg-secondary mb-4" style="width:1263px; height:8px;"></div>
      <div class="ms-5">
        <center>
          <h2>Form Soal Seleksi Amik Purnama Niaga Indramayu</h2>
        </center>
        <a href="" class="btn btn-primary ms-5 mb-2" type="submit" onclick="toggleElement()"><i class="bi bi-card-text"></i> Add</a>
      </div>   
      <table id="myScrollTable" border="0" cellpadding="5" cellspacing="4" align="center">
        <tbody class="rounded">
              <tr bgcolor="#2a7ed2" style="color:#ffffff;">
                <th style="width:60px; text-align:center;">No</th>
                <th style="width:550px;">Soal</th>
                <th style="width:150px; text-align:center;">Jawab A</th>
                <th style="width:150px; text-align:center;">Jawab B</th>
                <th style="width:150px; text-align:center;">Jawab C</th>
                <th style="width:150px; text-align:center;">Jawab D</th>
                <th style="width:60px; text-align:center;">Kunci Jawaban</th>
                <th style="width:60px; text-align:center;">Aksi</th>
              </tr>
            <?php
                include "koneksi.php";
                error_reporting(0);
                $data_user= "SELECT * FROM soal_seleksi";
                ...
                $no = 1;
                    ...
                    ?>
                <tr bgcolor="#7fb5ec">
                  <td class="align-text-top" style="width:60px;"><?php echo $no++; ?></td>
                  <td><?php echo $data['soal']; ?></td>
                  <td style="text-align:center;"><?php echo $data['a']; ?></td>
                  <td style="text-align:center;"><?php echo $data['b']; ?></td>
                  <td style="text-align:center;"><?php echo $data['c']; ?></td>
                  <td style="text-align:center;"><?php echo $data['d']; ?></td>
                  <td style="text-align:center;"><?php echo $data['kunci']; ?></td>
                  <td style="text-align:center;">
                    <a href="edit_soal.php?id=<?= $data['id_soal'];?>" class="btn btn-primary" type="submit" ><i class="bi bi-pencil-square"></i></a>
                  </td>
                </tr>
                <?php
                }
    ...
    <div id="elemen" style="position: absolute; display:block; width:700px; height:500px; margin-left:260px; margin-top:-370px;">
          <div id="elemen" class="alert alert-info alert-dismissible fade show" role="alert">
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            <form action="" method="POST">           
                <div class="mb-3">
                        <label for="formGroupExampleInput2" class="form-label">Soal</label>
                        <input type="text" class="form-control" id="formGroupExampleInput2" required name="soal">
                    </div>
                    <div class="d-flex">
                        <div class="mb-3">
                            <label for="formGroupExampleInput2" class="form-label">Jawab A</label>
                            <input type="text" class="form-control" id="formGroupExampleInput2" required name="a">
                        </div>
                        <div class="mb-3">
                            <label for="formGroupExampleInput2" class="form-label">Jawab B</label>
                            <input type="text" class="form-control" id="formGroupExampleInput2" required name="b">
                        </div>
                        <div class="mb-3">
                            <label for="formGroupExampleInput2" class="form-label">Jawab C</label>
                            <input type="text" class="form-control" id="formGroupExampleInput2" required name="c">
                        </div>
                        <div class="mb-3">
                            <label for="formGroupExampleInput2" class="form-label">Jawab D</label>
                            <input type="text" class="form-control" id="formGroupExampleInput2" required name="d">
                        </div>
                    </div>
                    <div class="mb-3">
                            <label for="formGroupExampleInput2" class="form-label">Kunci Jawaban</label>
                            <select class="form-select" aria-label="Default select example" name="kunci" required>
                    ...
                    <div class="d-flex">
                    <button class="btn btn-outline-warning me-1" type="submit" name="add">Add</button>
                    </div>
                <?php
                  include "koneksi.php";
                  if(isset($_POST['add'])){
                    ...
                    $data_soal = mysqli_query($con,"SELECT * FROM soal_seleksi WHERE soal='$soal' ");

                    // menghitung jumlah data_soal yang ditemukan
                    $cek_soal = mysqli_num_rows($data_soal);
                    
                    if($cek_soal > 0){
                        echo "<script>alert('gagal buat soal brow!');</script>";
                    }else{
                        $sql_soal = mysqli_query($con,"INSERT INTO soal_seleksi (soal, a, b, c, d, kunci, tanggal)
                        VALUES('$soal', '$a', '$c', '$c', '$d', '$kunci', '$tanggal')");
                
                        if($sql_soal){
                            echo "<script>alert('Succes Buat Soal!');window.location.href='soal.php'</script>";
                        }else{
                            echo "<script>alert('maaf anda gagal buat soal, silahkan coba lagi!');</script>";
                        }
                    }
                  }
                ?>
